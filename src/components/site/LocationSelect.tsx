...
function BrandMark({ brand }: { brand: string }) {
  return (
    <span className="flex h-8 w-[88px] shrink-0 items-center justify-end text-right font-mono text-[9px] leading-tight tracking-[0.06em] text-black/50 uppercase dark:text-white/50">
      {brand}
    </span>
  );
}
  ...
  return (
    <div className="relative">
      ...
      <button
        ...
        className="mt-2 flex w-full items-center justify-between gap-3 rounded-md border border-black/15 bg-black/5 px-3 py-2.5 text-left transition-colors hover:border-tnt-amber/60 focus-visible:ring-2 focus-visible:ring-tnt-amber focus-visible:outline-none dark:border-white/15 dark:bg-white/5"
      >
        ...
        <span id="nav-location-value" className="min-w-0">
          <span className="block truncate font-body text-sm font-semibold text-black dark:text-white">
            {selected.label}
          </span>
      ...
      {open && (
        ...
        <div className="absolute left-0 z-10 mt-1 w-[21rem] overflow-hidden rounded-md border border-black/15 bg-white shadow-2xl shadow-black/20 dark:border-white/15 dark:bg-black">
          <div className="border-b border-black/10 p-2 dark:border-white/10">
            <div className="flex items-center gap-2 rounded-md bg-black/5 px-2.5 py-2 ring-1 ring-black/10 focus-within:ring-tnt-amber dark:bg-white/5 dark:ring-white/10">
              <Search aria-hidden="true" className="h-4 w-4 shrink-0 text-black/50 dark:text-white/50" />
              <input
                ref={inputRef}
                ...
                className="w-full bg-transparent font-body text-sm text-black placeholder:text-black/40 focus:outline-none dark:text-white dark:placeholder:text-white/40"
              />
            </div>
          </div>
          ...
          >
            {filtered.length === 0 && (
              <li className="px-3 py-3 font-body text-sm text-black/50 dark:text-white/50">
              ...
              >
                <span
                  className={`min-w-0 flex-1 truncate font-body text-sm font-semibold ${
                    isSelected ? "text-tnt-amber" : "text-black/85 dark:text-white/85"
                  }`}
                >
                  {loc.label}
                </span>
